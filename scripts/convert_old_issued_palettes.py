"""Read issued_info.json and re-create the old palettes from issued_with_source.
...
from palette_image.color_block_ops import ColorBlocks, _get_input_arg_string
from palette_image import geometry as geo
from ..convert_classic import create_issued_info
...
#         infos = json.load(f)
#     for info in infos:
#         print(_format_title(info["title"]))
#         if len(info["colors"]) == 6:
#             groups = [1, 1, 1, 1, 2]
#         elif len(info["colors"]) == 7:
#             groups = [1, 1, 1, 1, 3]
#         else:
#             msg = f"Unexpected number of colors: {len(info['colors'])}"
#             raise ValueError(msg)
...
#             [height] * 5,
#             groups,
#             _get_input_arg_string(info["colors"], None)
#         )
#         # write_palette(info["source"], color_blocks, Path(info["source"]).with_suffix(".svg"))
